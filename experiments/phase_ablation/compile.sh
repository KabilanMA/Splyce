...
# In-process hardware counter helper (see perf_helper.c) — compiled once,
# linked into every binary below.
echo "[perf_helper] Compiling ..."
gcc -O2 -fPIE -c ./perf_helper.c -o ./perf_helper.o
...
compile_llvm_mlir() {
  ...
  clang "${CLANG_FLAGS[@]}" "$ll" ./perf_helper.o -o "$out_bin"
}
...
rm -f ./spgemm_scf.mlir ./spgemm_llvm_scf.mlir ./spgemm_llvm_scf.ll ./perf_helper.o
for phase in "${PHASES[@]}"; do
  rm -f \
