...
#   - one Splyce-vectorized binary, phase-select 001 (no --splyce-fastmath)
#     — the same single "the Splyce binary" phase choice used by
#     multicore/compile.sh, since this experiment only needs one
#     vectorized configuration to compare against the baseline across
#     sparsity levels, not a full phase-select sweep (see phase_ablation/
#     compile.sh for that).
#
# This script only compiles; it's meant to be driven by another script that
# regenerates data at each sparsity level and runs/benchmarks these
# binaries against it (see run.sh in this directory).
#
...
# ---------------------------------------------------------------------------
# 1. Sparsify -> SCF (shared input for the Splyce phase-001 variant below)
# ---------------------------------------------------------------------------
