#!/usr/bin/env python3
# plot_results.py — Plot Splyce speedup on real-world SpMTTKRP datasets.
#
# Reads spmttkrp_realworld_sweep_results.csv (produced by
...
# Usage:
#   ./plot_results.py
#       Reads ./spmttkrp_realworld_sweep_results.csv and writes both
#       spmttkrp_realworld_speedup_by_density.pdf (highest matrix nnz
#       density left to lowest right) and
#       spmttkrp_realworld_speedup_by_speedup.pdf (highest speedup left to
#       lowest right).
...
def nnz_density(row):
    ...
    # is just its complement. This is the real SuiteSparse matrix M's own
    # density (shared by tensor_C/tensor_D here), not tensor_B's — tensor_B
    # is synthetic, floored at its own density independently (see
    # run_suitesparse_sweep.py's TENSOR_B_DENSITY_FLOOR).
...
def main():
    args = sys.argv[1:]
    csv_path = os.path.join(SCRIPT_DIR, "spmttkrp_realworld_sweep_results.csv")
    out_path = None
    mode = "multicore"
    sort_by = "density"
    if "--csv" in args:
    ...
    if out_path is not None:
        ...
        plot(*data, sort_by="density",
             out_path=os.path.join(SCRIPT_DIR, "spmttkrp_realworld_speedup_by_density.pdf"))
        plot(*data, sort_by="speedup",
             out_path=os.path.join(SCRIPT_DIR, "spmttkrp_realworld_speedup_by_speedup.pdf"))
