#!/usr/bin/env python3
# print_results.py — Print vector_width/run.sh's results.csv (columns:
...
# exec_time_s divided by that same level's splyce_vw_N exec_time_s (1.00x
# on the baseline row itself).
#
# Usage:
...
def config_rank(config):
    # scf first, then splyce_vw_2/4/8/16 in ascending vector-width order
    # (not alphabetical — "splyce_vw_16" would otherwise sort before
    # "splyce_vw_2" as a string).
    if config == BASELINE_CONFIG:
        return -1
    if config.startswith("splyce_vw_"):
        try:
            return int(config.rsplit("_", 1)[1])
        except ValueError:
            pass
    return 10**9
...
def print_table(path):
    ...
    # Ascending sparsity level; baseline then ascending vector-width within it.
    data_rows.sort(key=lambda row: (float(row[sparsity_idx]), config_rank(row[config_idx])))
