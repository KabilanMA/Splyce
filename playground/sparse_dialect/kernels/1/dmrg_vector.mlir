                  ...
                  %81 = scf.if %80 -> (f64) {
                    ...
                    ^bb0(%arg46: index, %arg47: index, %arg48: f64):
                      ...
                      %103 = scf.if %102 -> (f64) {
                        ...
                        // ── SIMD fast path: runs while BOTH rows have ≥ 4 elements remaining ──
                        %vr:3 = scf.while (%arg49 = %111, %arg50 = %114, %arg51 = %arg48) : (index, index, f64) -> (index, index, f64) {
                          %c4     = arith.constant 4 : index
                          %118    = arith.addi %arg49, %c4 : index
                          %119    = arith.cmpi ule, %118, %113 : index  // arg49 + 4 <= end_D
                          %120    = arith.addi %arg50, %c4 : index
                          %121    = arith.cmpi ule, %120, %116 : index  // arg50 + 4 <= end_C
                          %122    = arith.andi %119, %121 : i1
                          scf.condition(%122) %arg49, %arg50, %arg51 : index, index, f64
                        } do {
                        ^bb0(%arg49: index, %arg50: index, %arg51: f64):
                          %zero_vec = arith.constant dense<0.000000e+00> : vector<4xf64>

                          // Load 4 coords and 4 values from each row (safe: ≥4 guaranteed by condition)
                          %d_cv = vector.load %subview_17[%arg49] : memref<?xindex>, vector<4xindex>
                          %d_vv = vector.load %subview_1[%arg49]  : memref<?xf64>,   vector<4xf64>
                          %c_cv = vector.load %subview_21[%arg50] : memref<?xindex>, vector<4xindex>
                          %c_vv = vector.load %subview_2[%arg50]  : memref<?xf64>,   vector<4xf64>

                          // Extract individual coords for pointer advancement and cross-compare
                          %d0 = vector.extract %d_cv[0] : index from vector<4xindex>
                          %d1 = vector.extract %d_cv[1] : index from vector<4xindex>
                          %d2 = vector.extract %d_cv[2] : index from vector<4xindex>
                          %d3 = vector.extract %d_cv[3] : index from vector<4xindex>
                          %cc0 = vector.extract %c_cv[0] : index from vector<4xindex>
                          %cc1 = vector.extract %c_cv[1] : index from vector<4xindex>
                          %cc2 = vector.extract %c_cv[2] : index from vector<4xindex>
                          %cc3 = vector.extract %c_cv[3] : index from vector<4xindex>

                          // 4x4 cross-comparison: row_i[j] = true iff D[i] == C[j]
                          %d0_b = vector.broadcast %d0 : index to vector<4xindex>
                          %row0 = arith.cmpi eq, %d0_b, %c_cv : vector<4xindex>
                          %d1_b = vector.broadcast %d1 : index to vector<4xindex>
                          %row1 = arith.cmpi eq, %d1_b, %c_cv : vector<4xindex>
                          %d2_b = vector.broadcast %d2 : index to vector<4xindex>
                          %row2 = arith.cmpi eq, %d2_b, %c_cv : vector<4xindex>
                          %d3_b = vector.broadcast %d3 : index to vector<4xindex>
                          %row3 = arith.cmpi eq, %d3_b, %c_cv : vector<4xindex>

                          // Masked C-value extraction per D element (0.0 if no match)
                          %cm0    = arith.select %row0, %c_vv, %zero_vec : vector<4xi1>, vector<4xf64>
                          %c_val0 = vector.reduction <add>, %cm0 : vector<4xf64> into f64
                          %cm1    = arith.select %row1, %c_vv, %zero_vec : vector<4xi1>, vector<4xf64>
                          %c_val1 = vector.reduction <add>, %cm1 : vector<4xf64> into f64
                          %cm2    = arith.select %row2, %c_vv, %zero_vec : vector<4xi1>, vector<4xf64>
                          %c_val2 = vector.reduction <add>, %cm2 : vector<4xf64> into f64
                          %cm3    = arith.select %row3, %c_vv, %zero_vec : vector<4xi1>, vector<4xf64>
                          %c_val3 = vector.reduction <add>, %cm3 : vector<4xf64> into f64

                          // Per-lane contribution: d_val_i * c_val_i  (0 when no match)
                          %d0_val   = vector.extract %d_vv[0] : f64 from vector<4xf64>
                          %d1_val   = vector.extract %d_vv[1] : f64 from vector<4xf64>
                          %d2_val   = vector.extract %d_vv[2] : f64 from vector<4xf64>
                          %d3_val   = vector.extract %d_vv[3] : f64 from vector<4xf64>
                          %contrib0 = arith.mulf %d0_val, %c_val0 : f64
                          %contrib1 = arith.mulf %d1_val, %c_val1 : f64
                          %contrib2 = arith.mulf %d2_val, %c_val2 : f64
                          %contrib3 = arith.mulf %d3_val, %c_val3 : f64

                          // Horizontal sum of contributions
                          %s01      = arith.addf %contrib0, %contrib1 : f64
                          %s23      = arith.addf %contrib2, %contrib3 : f64
                          %s_total  = arith.addf %s01, %s23 : f64

                          // Scale by outer factors and accumulate into carried acc
                          %scale0   = arith.mulf %42, %110 : f64
                          %scale1   = arith.mulf %scale0, %88 : f64
                          %scaled   = arith.mulf %scale1, %s_total : f64
                          %new_acc  = arith.addf %arg51, %scaled : f64

                          // Branchless pointer advancement via pivot = min(D_max, C_max)
                          %pivot    = arith.minui %d3, %cc3 : index

                          %d0_le  = arith.cmpi ule, %d0, %pivot : index
                          %d1_le  = arith.cmpi ule, %d1, %pivot : index
                          %d2_le  = arith.cmpi ule, %d2, %pivot : index
                          %d3_le  = arith.cmpi ule, %d3, %pivot : index
                          %d0_adv = arith.select %d0_le, %c1, %c0 : index
                          %d1_adv = arith.select %d1_le, %c1, %c0 : index
                          %d2_adv = arith.select %d2_le, %c1, %c0 : index
                          %d3_adv = arith.select %d3_le, %c1, %c0 : index
                          %d_s01  = arith.addi %d0_adv, %d1_adv : index
                          %d_s23  = arith.addi %d2_adv, %d3_adv : index
                          %d_step = arith.addi %d_s01,  %d_s23  : index
                          %new_arg49 = arith.addi %arg49, %d_step : index

                          %c0_le  = arith.cmpi ule, %cc0, %pivot : index
                          %c1_le  = arith.cmpi ule, %cc1, %pivot : index
                          %c2_le  = arith.cmpi ule, %cc2, %pivot : index
                          %c3_le  = arith.cmpi ule, %cc3, %pivot : index
                          %c0_adv = arith.select %c0_le, %c1, %c0 : index
                          %c1_adv = arith.select %c1_le, %c1, %c0 : index
                          %c2_adv = arith.select %c2_le, %c1, %c0 : index
                          %c3_adv = arith.select %c3_le, %c1, %c0 : index
                          %c_s01  = arith.addi %c0_adv, %c1_adv : index
                          %c_s23  = arith.addi %c2_adv, %c3_adv : index
                          %c_step = arith.addi %c_s01,  %c_s23  : index
                          %new_arg50 = arith.addi %arg50, %c_step : index

                          scf.yield %new_arg49, %new_arg50, %new_acc : index, index, f64
                        } attributes {"Emitted from" = "linalg.generic"}

                        // ── Scalar tail: handles remaining 0-3 elements in either row ──
                        // Picks up exactly where the SIMD loop left off (%vr#0, %vr#1, %vr#2)
                        %117:3 = scf.while (%arg49 = %vr#0, %arg50 = %vr#1, %arg51 = %vr#2) : (index, index, f64) -> (index, index, f64) {
